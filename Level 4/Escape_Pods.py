...
the numerous bunnies through the various rooms to the escape pods. Unfortunately
, the corridors between the rooms can only fit so many bunnies at a time. What's
more, many of the corridors were resized to accommodate the LAMBCHOP, so they
vary in how many bunnies can move through them at a time. 
...
def solution(entrances, exits, path):
    ...
    residual = [list(p) + [0, 0] for p in path]
    max_flow = 0
    ...
    for v in entrances:
        residual[SOURCE][v] = 2000000

    for u in exits:
        residual[u][SINK] = 2000000
